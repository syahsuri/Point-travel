 ...
 *
 * Mobile renders a collapsible bottom sheet (isMinimized controls a summary
 * bar vs the full detail). Desktop always renders the full detail — it is
 * NOT gated by isMinimized, since that state/toggle only exists for mobile.
 */
export default function AirportDetailSidebar({
  ...
  // Shared "full detail" content — header, photo, info rows, tabs, schedule.
  // Rendered once inside the mobile sheet (behind isMinimized) and once,
  // unconditionally, inside the desktop panel.
  const fullDetail = (
  ...
  return (
    <>
      {/* ---------- MOBILE: collapsible bottom sheet (< md) ---------- */}
      <div className="md:hidden absolute z-20 flex flex-col overflow-hidden bg-black/85 text-xs text-white/85 backdrop-blur-md shadow-2xl transition-all bottom-0 left-0 w-full rounded-t-2xl border-t border-white/15">
        {/* Drag handle bar */}
        <button
          type="button"
      ...
      {/* ---------- DESKTOP: always-open panel (>= md) ---------- */}
      <div className="hidden md:flex absolute z-20 left-4 top-16 w-72 max-h-[calc(100dvh-5rem)] flex-col overflow-hidden rounded-md border border-sky-400/20 bg-black/70 text-xs text-white/85 backdrop-blur-md shadow-2xl">
        {fullDetail}
      </div>
    </>
  );
